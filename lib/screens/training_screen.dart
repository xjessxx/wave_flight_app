...
class _TrainingScreenState extends State<TrainingScreen>
  ...
  void _runNextTrial() {
    if (!_trainingActive || _currentTrial >= _totalTrials) return;
  ...
  Future<void> _triggerJump() async {
    ...
    if (_trainingActive) {
      final ok = await _bciService.trainingTrialStart();
      if (!ok) {
        debugPrint('Failed to notify Python about training trial start.');
      }
    }
  }
        ...
        children: [
          ...
          AnimatedBuilder(
            ...
            builder: (context, child) {
              ...
              if (animationProgress <= 0.40) {
                final shootProgress = animationProgress / 0.40;
                final arcHeight = _calculateArcHeight(shootProgress);
                final distanceToTravel = screenHeight - 300;
                final forwardMovement = shootProgress * distanceToTravel;
                final maxArcHeight = 300.0;
                final arcOffset = arcHeight * maxArcHeight;
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: const Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(Icons.psychology, color: Color(0xFF4A90E2), size: 20),
                    SizedBox(width: 8),
                    Text(
                      'BCI ACTIVE',
                      style: TextStyle(
